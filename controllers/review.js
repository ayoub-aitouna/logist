...
const AddReview = async (req, res) => {
	...
	try {
		const Review = await Mysql(
			`INSERT into Reviews(driver_id , user_id , Rating, review_date)VALUES(?,?,?,CURDATE());`,
			[DriverId, UserId, Rating]
		);
...
const Reviews = async (req, res) => {
	...
	try {
		const Review = await Mysql("Select * from Reviews where driver_id = ?", [
			DriverId,
		]);
		res.json(Review);
	} catch (err) {
